...
const history = document.getElementById("recentlyViewed");
...
if (history) {
    ...
    if (ids.length === 0) {
        const messageToStore = document.createElement("p");
        messageToStore.textContent = "You haven't viewed any of our pieces, click on the store link above to go check them out!"
        history.appendChild(messageToStore);
    }
    else {
        const filteredHistory = product.filter(product => ids.includes(product.pn));

        filteredHistory.forEach(product => {
            ...
            img.alt = `Picture of product number ${product.pn} which is a ${product.shape} shape.`;
            img.loading = "lazy";
